...
void PartialPath(int utype, int umode, int cmstartx, int cmstarty, int target, int target2, int targtype, int cdtype,
                 ...
                 int cmgoalx, int cmgoaly, int cmgoalminx, int cmgoalminy, int cmgoalmaxx, int cmgoalmaxy,
                 int maxsearch, bool capend, bool allowpart, bool adjgoalbounds)
{
	...
	if(!ignoreb)
	{
		...
		case UMODE_GOREST:
		case UMODE_RESTING:
			//ignoreb = &g_building[target];	//corpd fix
			if(thisu)
				ignoreb = &g_building[thisu->home];
			break;
		case UMODE_GOSUP:
		case UMODE_ATSUP:
			if(thisu)
...
void Expand_QP(PathJob* pj, PathNode* node)
{
	...
	for(unsigned char i=0; i<DIRS; i++)
	{
		...
		Log("\t not pass n "<<nextnpos.x<<","<<nextnpos.y);
#endif
