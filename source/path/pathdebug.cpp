...
void DrawGrid()
{
	...
	if(g_sel.units.size() > 0)
	{
		...
		for(int x=imax(0, ux-50); x<imin(ux+50, g_pathdim.x); x++)
			for(int y=imax(0, uz-50); y<imin(uz+50, g_pathdim.y); y++)
			{
	...
	for(int y=0; y<g_pathdim.y-1; y++)
	{
		for(int x=0; x<g_pathdim.x-1; x++)
		{
			...
			//glVertex3f(g_map.m_widthX*TILE_SIZE, 0 + 1, y*(MIN_RADIUS*2.0f));
		}
	}
