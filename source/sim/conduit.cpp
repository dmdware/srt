...
#include "../gui/layouts/gviewport.h"
...
bool CdLevel(unsigned char ctype, float iterx, float itery, float testx, float testy, float dx, float dy, int i, float d, bool plantoo)
{
	//return true;
...
bool CdPlaceable(int ctype, int x, int y)
{
	CdType* ct = &g_cdtype[ctype];
...
int GetConn(unsigned char ctype, int x, int y, bool plan=false)
{
	bool n = false, e = false, s = false, w = false;
...
void ConnectCd(unsigned char ctype, int tx, int ty, bool plan)
{
...
void PlaceCd(PlaceCdPacket* pcp)
{
	...
	char cmess[128];
	sprintf(cmess, " placed %d %ss.", (int)pcp->ntiles, ct->name);
	RichText mess = name + RichText(cmess);
	AddChat(&mess);
...
void DrawCd(unsigned char ctype)
{
	...
	for(int x=0; x<g_mapsz.x; x++)
		for(int y=0; y<g_mapsz.y; y++)
		{
			...
			int spi = ct->sprite[ctile->conntype][1-(int)ctile->finished][tile.incltype];
			Sprite* sp = &g_sprite[ spi ];
