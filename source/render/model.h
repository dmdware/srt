#ifndef MODEL_H
#define MODEL_H

#include "../platform.h"
#include "ms3d.h"
#include "../math/vec3f.h"
#include "../math/vec2f.h"

class VertexArray;
class MS3DModel;
class Shader;

class Model
{
public:
	bool m_on;
	MS3DModel m_ms3d;
	//VertexArray* m_va;
	int m_nframes;
	unsigned int m_diffusem;
	unsigned int m_specularm;
	unsigned int m_normalm;
	unsigned int m_ownerm;
	std::string m_fullpath;
	Vec3f m_scale;
	Vec3f m_translate;

	Model()
	{
		//m_va = NULL;
		m_on = false;
	}

	~Model();

	bool load(const char* relative, Vec3f scale, Vec3f translate, bool dontqueue, bool blendnorm);
	void genvbo();
	void delvbo();
	void usedifftex() const;
	void usespectex() const;
	void usenormtex() const;
	void useteamtex() const;
	void draw(int frame, Vec3f pos, float yaw, VertexArray* va);
	void destroy();
};

#define MODELS	512
extern Model g_model[MODELS];

struct ModelToLoad
{
	int* id;
	char filepath[DMD_MAX_PATH];
	Vec3f scale;
	Vec3f translate;
	bool blendnorm;
};

int NewModel();
int FindModel(const char* relative);
void QueueModel(int* id, const char* relative, Vec3f scale, Vec3f translate, bool blendnorm=false);
bool Load1Model();
void DrawVA(VertexArray* va, Vec3f pos);
void DrawVADepth(VertexArray* va, Vec3f pos);
void BeginVertexArrays();
void EndVertexArrays();
#if 1
bool PlayAnif(float& frame, int first, int last, bool loop, float rate);
bool PlayAniBf(float& frame, int first, int last, bool loop, float rate);	//Play animation backwards
#endif
void FreeModels();
int LoadModel(const char* relative, Vec3f scale, Vec3f translate, bool dontqueue, bool blendnorm);

#endif
