...
{
	char front[DMD_MAX_PATH+1];
	char left[DMD_MAX_PATH+1];
	char right[DMD_MAX_PATH+1];
	char bottom[DMD_MAX_PATH+1];
	char top[DMD_MAX_PATH+1];
	char back[DMD_MAX_PATH+1];
...
void DrawSkyBox(Vec3f pos)
{
	...
	//g_applog<<"mm"<<s->slot[SSLOT_MODELMAT]<<std::endl;
#if 1
	Matrix modelmat;
...
//typedef void (GLAPIENTRY * PFNGLVERTEXATTRIBPOINTERPROC) (GLuint index, GLint size, GLenum type, GLboolean normalized, GLsizei stride, const GLvoid* pointer);
	...
	glVertexAttribPointer(s->slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vright[2]);

	CheckGLError(__FILE__, __LINE__);

	glVertexAttribPointer(s->slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vright[0]);

	//g_applog<<"tci"<<s->slot[SSLOT_TEXCOORD0]<<std::endl;
	...
	glVertexAttribPointer(s->slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vleft[2]);
	glVertexAttribPointer(s->slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vleft[0]);
	glDrawArrays(GL_TRIANGLES, 0, 6);
	...
	glVertexAttribPointer(s->slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vfront[2]);
	glVertexAttribPointer(s->slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vfront[0]);
	glDrawArrays(GL_TRIANGLES, 0, 6);
	...
	glVertexAttribPointer(s->slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vback[2]);
	glVertexAttribPointer(s->slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vback[0]);
	glDrawArrays(GL_TRIANGLES, 0, 6);
	...
	glVertexAttribPointer(s->slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vbottom[2]);
	glVertexAttribPointer(s->slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vbottom[0]);
	glDrawArrays(GL_TRIANGLES, 0, 6);
	...
	glVertexAttribPointer(s->slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vtop[2]);
	glVertexAttribPointer(s->slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(float)*5, &vtop[0]);
	glDrawArrays(GL_TRIANGLES, 0, 6);
