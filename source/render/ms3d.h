...
{
	public:
		char m_relative[DMD_MAX_PATH+1];
		int m_frame;
	...
	public:
		...
		bool load(const char *relative, unsigned int& diffm, unsigned int& specm, unsigned int& normm, unsigned int& ownm, Vec3f scale, Vec3f translate, bool dontqueue);
		void destroy();

		void loadtex(unsigned int& diffm, unsigned int& specm, unsigned int& normm, unsigned int& ownm, bool dontqueue);
		void genva(VertexArray* va, int f, Vec3f scale, Vec3f translate, bool blendnorms);
		void genva_aimer(VertexArray* va, int upf, int lowf, float pitch, Vec3f scale, Vec3f translate, bool blendnorms);
