...
"to reset" pathnodes as closed nodes, and show list,
...
Problem is it will take a long time to learn a
...
since path search progress from
previous frame must be memory
...
{
 long hash = this->Hash(inNode->mX, inNode->mY);
...
with only the 15 cheapest show nodes.
When cheap list runs out,
select the 15 cheapest items from
show list and heapify them.
...
show list for lowest cost item.
...
to an show node that is shorter;
...
void VisitNode(Depthable* d, std::list<Depthable*>& drawlist, std::list<Depthable*>& drawqueue)
{
	...
	for(auto subd=d->behind.begin(); subd!=d->behind.end(); subd++)
	{
		if(*subd == NULL)
			break;
		else
...
void DrawSort(std::list<Depthable*>& drawlist, std::list<Depthable*>& drawqueue)
{
	...
	for(auto dit=drawlist.begin(); dit!=drawlist.end(); dit++)
	{
		Depthable* d = *dit;

		for(auto dit2=drawlist.begin(); dit2!=drawlist.end(); dit2++)
		{
			Depthable* d2 = *dit2;
