...
void MS3DModel::loadtex(unsigned int& diffm, unsigned int& specm, unsigned int& normm, unsigned int& ownm, bool dontqueue)
{
	for ( int i = 0; i < m_numMaterials; i++ )
		if ( strlen( m_pMaterials[i].m_pTextureFilename ) > 0 )
		{
			...
			char basefile[DMD_MAX_PATH+1];
			strcpy(basefile, m_pMaterials[i].m_pTextureFilename);
			StripPath(basefile);
			char basename[DMD_MAX_PATH+1];
			sprintf(basename, "%s%s", m_relative, basefile);
			char difffile[DMD_MAX_PATH+1];
			strcpy(difffile, basename);
			StripExt(basename);
			char specfile[DMD_MAX_PATH+1];
			char normfile[DMD_MAX_PATH+1];
			char ownfile[DMD_MAX_PATH+1];
...
bool MS3DModel::load(const char *relative, unsigned int& diffm, unsigned int& specm, unsigned int& normm, unsigned int& ownm, Vec3f scale, Vec3f translate, bool dontqueue)
{
	...
	*/
	...
	for ( i = 0; i < m_numJoints; i++ )
	{
		...
		if(strcmp(pJoint->m_name, "Bip01 Spine3") == 0)
			m_aimpitchjoint = i;
	}
	...
	for ( i = 0; i < m_numJoints; i++ )
	{
		...
		for ( j = 0; j < pJoint->m_numTranslationKeyframes; j++ )
		{
			...
			//scale and translate ...
			Keyframe& keyframe = m_pJoints[i].m_pTranslationKeyframes[j];
			keyframe.m_parameter[0] *= scale.x;
			keyframe.m_parameter[1] *= scale.y;
			keyframe.m_parameter[2] *= scale.z;
			keyframe.m_parameter[0] += translate.x;
			keyframe.m_parameter[1] += translate.y;
			keyframe.m_parameter[2] += translate.z;
		}
	}
	...
	for ( i = 0; i < nVertices; i++ )
	{
		...
		m_pVertices[i].m_location[0] += translate.x;
		m_pVertices[i].m_location[1] += translate.y;
		m_pVertices[i].m_location[2] += translate.z;
	}

	for ( i = 0; i < m_numJoints; i++ )
	{
		m_pJoints[i].m_localTranslation[0] *= scale.x;
		m_pJoints[i].m_localTranslation[1] *= scale.y;
		m_pJoints[i].m_localTranslation[2] *= scale.z;
		m_pJoints[i].m_localTranslation[0] += translate.x;
		m_pJoints[i].m_localTranslation[1] += translate.y;
		m_pJoints[i].m_localTranslation[2] += translate.z;
	}
...
void MS3DModel::genva(VertexArray* va, int f, Vec3f scale, Vec3f translate, bool blendnorms)
{
...
void MS3DModel::genva_aimer(VertexArray* va, int upf, int lowf, float pitch, Vec3f scale, Vec3f translate, bool blendnorms)
{
}
