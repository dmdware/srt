...
void Click_SL_LANRef()	//LAN refresh
{
	...
	for(int i=0; i<10; i++)
	{
		...
		//unsigned int broadip = (unsigned int)-1;
		unsigned short port = PORT + i;
...
void Click_SL_QRef()	//qk refresh
{
	...
	for(auto sit=v->m_svlist.begin(); sit!=v->m_svlist.end(); sit++)
	{
		...
		SendData((char*)&gsip, sizeof(GetSvInfoPacket), &sv->addr, true, false, Match(&sv->addr), &g_sock, 100, NULL);
	}
}
...
SvList::SvList(Widget* parent, const char* n, void (*reframef)(Widget* w)) : Win(parent, n, reframef)
{
	...
	CreateTex(m_texaccept, "gui/accept.png", true, false);
	CreateTex(m_texcancel, "gui/cancel.png", true, false);
...
#if 0
		Button(Widget* parent, const char* name, const char* filepath, const RichText label, const RichText tooltip,int f, int style, void (*reframef)(Widget* w), void (*click)(), void (*click2)(int p), void (*overf)(), void (*overf2)(int p), void (*out)(), int parm);
#endif
...
void SvList::reframe()
{
	...
	m_refbut.m_pos[0] = m_pos[2] - 130 - 10 - 70 - 10 - 130 - 10 - 70;
	...
	m_lanrefbut.m_pos[0] = m_pos[2] - 130 - 10 - 70 - 10 - 130 - 10 - 70 - 140;
	m_lanrefbut.m_pos[1] = m_pos[3] - 30;
	m_lanrefbut.m_pos[2] = m_pos[2] - 10 - 130 - 10 - 130 - 10 - 70 - 80;
	m_lanrefbut.m_pos[3] = m_pos[3];
...
void SvList::draw()
{
	...
	if(g_reqsvlist && g_reqdnexthost)
	{
		if(!g_mmconn || !g_mmconn->handshook)
		{
			...
			DrawShadowedText(MAINFONT16, m_pos[0], m_pos[1], &stat);
		}
		else
		{
	...
	if(m_svlist.size() <= 0)
	{
			...
			DrawShadowedText(MAINFONT16, m_pos[0], m_pos[1] + f->gheight, &stat);
	}
	...
	for(auto sit=m_svlist.begin(); sit!=m_svlist.end(); sit++, sin++)
	{
		...
		if(sit->started)
			DrawImage(g_texture[m_texcancel].texname, m_pos[0], (int)(m_listtop + (sin-scroll)*f->gheight), m_pos[0] + f->gheight, (int)(m_listtop + (sin-scroll+1)*f->gheight), 0, 0, 1, 1, m_crop);
		else
			DrawImage(g_texture[m_texaccept].texname, m_pos[0], (int)(m_listtop + (sin-scroll)*f->gheight), m_pos[0] + f->gheight, (int)(m_listtop + (sin-scroll+1)*f->gheight), 0, 0, 1, 1, m_crop);

		float namecolor[4] = {0.9f, 0.6f, 0.2f, 0.9f};
		DrawShadowedTextF(m_font, m_pos[0] + 32, (int)(m_listtop + (sin-scroll)*f->gheight), m_pos[0], m_pos[1], m_pos[2], m_listbot, &sit->name, namecolor);
