...
void CstrView::regen(Selection* sel)
{
	...
	else if(sel->powls.size() > 0)
	{
		...
		for(auto cit=sel->powls.begin(); cit!=sel->powls.end(); cit++)	//corpc fix
		{
	...
	else if(sel->crpipes.size() > 0)
	{
		...
		for(auto cit=sel->crpipes.begin(); cit!=sel->crpipes.end(); cit++)	//corpc fix
		{
	...
	add(new Text(this, rowname, STRTABLE[STR_CONWAGE] + RichText(RichPart(RICH_ICON, ICON_LABOUR)) + RichText(UStr(":")), MAINFONT16, Resize_CV_Cl, true, 1.0f,1.0f,1.0f,1.0f /*, 0.7f, 0.9f, 0.3f, 1*/));
