...
float Bilerp(Heightmap* hmap, float x, float y)
{
	...
	float hR1 = hmap->getheight(x1,y1)*x2fac + hmap->getheight(x2,y1)*x1fac;
	float hR2 = hmap->getheight(x1,y2)*x2fac + hmap->getheight(x2,y2)*x1fac;
...
bool MapInter(Heightmap* hmap, Vec3f ray, Vec3f point, Vec3i* cmint)
{
	...
	//g_applog<<"start "<<point.x<<","<<point.y<<","<<point.z<<" ray*minratio="<<minray.x<<","<<minray.y<<","<<minray.z<<std::endl;
		...
		lastpoint = point, point = point - minray)
	{
		...
		//g_applog<<"point "<<point.x<<","<<point.y<<","<<point.z<<" tile "<<(int)tx<<","<<(int)ty<<", zrange="<<miny<<","<<maxy<<std::endl;
...
bool TileIntersect(Heightmap* hmap, Vec2uc sz, Vec3f* line, int tx, int ty, Vec3f* intersect)
{
    Vec3f tri[3];
    const int wx = sz.x;
    //const int wy = sz.y;
    const Vec3f* v = hmap->m_3dverts;
