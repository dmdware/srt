...
#define OS_IN_OUTPUTPOS(a,b) \
	...
	outputpos += (a)*(b);
            ...
            for (i = 0; i < formatcount; i++)
            {
                ...
                case 'h':
                case 'H':
                    OS_IN_OUTPUTPOS((arg + (arg % 2)) / 2, 1)	/* 4 bit per arg */
                    break;
                ...
                case 'C':
                case 'x':
                    OS_IN_OUTPUTPOS(arg, 1)		/* 8 bit per arg */
                    break;
                ...
                case 'n':
                case 'v':
                    OS_IN_OUTPUTPOS(arg, 2)		/* 16 bit per arg */
                    break;

                case 'i':
                case 'I':
                    OS_IN_OUTPUTPOS(arg, sizeof(int))
                    break;
                ...
                case 'N':
                case 'V':
                    OS_IN_OUTPUTPOS(arg, 4)		/* 32 bit per arg */
                    break;

                case 'f':
                    OS_IN_OUTPUTPOS(arg, sizeof(float))
                    break;

                case 'd':
                    OS_IN_OUTPUTPOS(arg, sizeof(double))
                    break;
...
bool OS::Core::File::show(const OS_CHAR * filename, const OS_CHAR * mode)
{
    hide();
...
void OS::initFileClass()
{
    struct Lib
    {
        static int __construct(OS * os, int params, int, int, void * user_param)
        {
            ...
            {
                self->show(os->toString(-params), os->toString(-params+1));
            }
            else if(params >= 1)
            {
                self->show(os->toString(-params));
            }
            CtypeValue<Core::File*>::push(os, self);
            return 1;
        }

        static int show(OS * os, int params, int, int, void * user_param)
        {
            OS_GET_SELF(Core::File*);
            if(params >= 2 && !os->isNull(-params+1))
            {
                os->pushBool(self->show(os->toString(-params), os->toString(-params+1)));
            }
            else if(params >= 1)
            {
                os->pushBool(self->show(os->toString(-params)));
            }
            else
            {
                self->hide();
    ...
    {
        {OS_TEXT("__construct"), Lib::__construct},
        {OS_TEXT("show"), Lib::show},
        def(OS_TEXT("hide"), &Core::File::hide),
...
bool OS::compileFile(const String& p_filename, bool required, OS_ESourceCodeType source_code_type, bool check_utf8_bom)
{
    ...
    if(!file.f)
    {
        setException(String::format(this, OS_TEXT("error show filename %s"), p_filename.toChar()));
        return false;
    }
...
void OS::require(const String& filename, bool required, int ret_values, OS_ESourceCodeType source_code_type, bool check_utf8_bom, bool handle_exception)
{
    ...
    g_applog<<filename.toChar()<<std::endl;
    g_applog.flush();
